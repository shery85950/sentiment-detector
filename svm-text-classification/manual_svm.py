...
# SVM functions
# ----------------------------
def score(x,w,b): return np.dot(x,w)+b
...
# Compute metrics and store for selecting best model
model_list = []
for name,(w,b) in models.items():
    p = predict(X_test,w,b)
    a, pr, re, f1 = metrics(y_test,p)
    print(f"{name} | Acc: {a:.4f} | F1: {f1:.4f}")
    model_list.append((name, (w,b), f1))

# Pick the best model by F1
best_name, best_model, best_f1 = max(model_list, key=lambda x:x[2])
joblib.dump(best_model, "best_model_manual.pkl")
print(f"Saved best model ({best_name}) with F1={best_f1:.4f} to best_model_manual.pkl")
